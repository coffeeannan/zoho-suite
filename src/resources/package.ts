...
import { formatDateToYMDHM } from '../util/format';
...
export class PackageResource {
   ...
   * @param input
   */
  public async bulkCreateQuickShipment(input: QuickCreateInput): Promise<void> {
    const chunkSize = 25;
  ...
  public async markDelivered(shipmentOrderId: string, time: Date) {
    const res = await this.client.post<{
      ...
      body: {
        delivered_date: formatDateToYMDHM(time),
      },
    });
  ...
  public async deleteShipmentOrder(ids: string[]): Promise<void> {
    ...
    if (ids.length === 1) {
      await this.client.delete({
        path: ['shipmentorders', ids[0] as string],
      });
      return;
    }
